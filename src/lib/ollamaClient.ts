    ...
    try {
      ...
      for (const ln of lines) {
        ...
        try {
          ...
          // Ollama NDJSON line has {response?: string, done?: boolean}
          if (obj.response) {
      ...
      if (isDone) {
        ...
        if (final) {
          try {
            ...
            if (obj.response) opts.onToken(obj.response);
          } catch {}
        }
...
export async function getModels(baseUrl: string): Promise<string[]> {
  const url = normalizeUrl(baseUrl) + '/api/models';
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Failed to fetch models: ${res.status}`);
  const json = await res.json();
  // Shape may be { models: [{name: '...'}] }
  if (Array.isArray(json.models)) return json.models.map((m: any) => m.name).filter(Boolean);
...
export async function ping(baseUrl: string): Promise<boolean> {
  ...
  // Fallback to /api/models if /api/version isn't available
  try {
